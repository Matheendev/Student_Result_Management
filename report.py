...
class reportClass:
    def __init__(self,root):
        ...
        title = Label(self.root, text="View Students Results",font=("goudy old style", 20, "bold"), bg="orange", fg="#262626").place(x=10, y=15, width=1180,height=50)

        self.var_search=StringVar()
        self.var_id=""
        lbl_search = Label(self.root, text="Search by Roll No:",font=("goudy old style", 20, "bold"), bg="white").place(x=280, y=100)
        txt_search = Entry(self.root, text=self.var_search,font=("goudy old style", 20), bg="white").place(x=500, y=100,width=150)
        btn_search= Button(self.root, text="Search", font=("goudy old style", 15, "bold"), bg="#2196f3", fg="white", cursor="hand2",command=self.search).place(x=660,y=100,width=110,height=35)
        btn_clear= Button(self.root, text="Clear", font=("goudy old style", 15, "bold"), bg="gray", fg="white", cursor="hand2",command=self.clear).place(x=780,y=100,width=110,height=35)
        ...
        btn_delete= Button(self.root, text="Delete", font=("goudy old style", 15, "bold"), bg="red", fg="white", cursor="hand2",command=self.delete).place(x=530,y=350,width=110,height=30)
    ...
    def delete(self):
        ...
        try:
            if self.var_id=="":
                ...
                row=cur.fetchone()
                if row ==None:
                    messagebox.showerror("Error","Invalisd Student Result", parent=self.root)
                else:
