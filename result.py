...
class resultClass:
    def __init__(self,root):
        ...
        title = Label(self.root, text=" Add Students Results",font=("goudy old style", 20, "bold"), bg="orange", fg="#262626").place(x=10, y=15, width=1180,height=50)
        ...
        btn_search= Button(self.root, text="Search", font=("goudy old style", 15, "bold"), bg="#2196f3", fg="white", cursor="hand2",command=self.search).place(x=500,y=105,width=110,height=35)
        txt_name = Entry(self.root, textvariable=self.var_name,font=("goudy old style", 15, "bold"), bg="white",state='readonly').place(x=280,y=160,width=340,height=35)
        txt_course = Entry(self.root, textvariable=self.var_course,font=("goudy old style", 15, "bold"), bg="white",state='readonly').place(x=280,y=220,width=340,height=35)
        txt_marks = Entry(self.root, textvariable=self.var_marks,font=("goudy old style", 15, "bold"), bg="white").place(x=280,y=280,width=340,height=35)
        txt_full_marks = Entry(self.root, textvariable=self.var_full_marks,font=("goudy old style", 15, "bold"), bg="white").place(x=280,y=340,width=340,height=35)
        ...
        self.bl_image = Image.open("images/boy.jpg")
        self.bl_image = self.bl_image.resize((500,300),Image.Resampling.BILINEAR)
        self.bl_image = ImageTk.PhotoImage(self.bl_image)
        self.lbl_image = Label(self.root,image=self.bl_image).place(x=630,y=70)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                messagebox.showerror("Error", "Please First Enter The Student Roll Number", parent=self.root)
            else:
